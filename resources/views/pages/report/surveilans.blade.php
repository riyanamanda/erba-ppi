                            ...
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <select id="jenis_laporan" name="jenis_laporan" class="form-control">
                                        ...
                                        <option value="semua" @selected(request()->get('jenis_laporan') == 'semua')>Semua</option>
                                        <option value="isk" @selected(request()->get('jenis_laporan') == 'isk')>Infeksi Saluran Kemih (ISK)</option>
                                        <option value="phlebitis" @selected(request()->get('jenis_laporan') == 'phlebitis')>Phlebitis</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-3">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" id="dari_tanggal" name="dari_tanggal" value="{{ request()->get('dari_tanggal') }}">
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="form-group">
                                    <label for="sampai_tanggal">Sampai Tanggal</label>
                                    <input type="date" class="form-control" id="sampai_tanggal" name="sampai_tanggal" value="{{ request()->get('sampai_tanggal') }}">
                                </div>
                            </div>
                            <div class="col-1">
            ...
            <div class="card">
                <div class="card-header align-items-center justify-content-between">
                    ...
                    <form action="{{ route('report.export') }}" method="POST">
                        ...
                        <input type="text" name="dari" value="{{ Carbon\Carbon::parse(request()->get('dari_tanggal'))->isoFormat('D MMMM Y') }}" hidden>
                        <input type="text" name="sampai" value="{{ Carbon\Carbon::parse(request()->get('dari_tanggal'))->isoFormat('D MMMM Y') }}" hidden>
                ...
                <div class="card-footer d-flex align-items-center justify-content-between">
                    <div class="d-flex flex-column">
                        ...
                        <span style="font-size: 12px;">Periode <strong class="text-primary">{{ Carbon\Carbon::parse(request()->get('dari_tanggal'))->isoFormat('D MMMM Y') }}</strong> - <strong class="text-primary">{{ Carbon\Carbon::parse(request()->get('sampai_tanggal'))->isoFormat('D MMMM Y') }}</strong>.</span>
                    </div>
                    <div style="font-size: 12px;"><strong class="text-primary">{{ $surveilans->count() }}</strong> total rekaman.</div>
                </div>
            </div>
        </div>
    </div>
